...
if(isset($_POST['submit']))
{
	...
	{
			$sql="UPDATE shopowner SET compname='$_POST[compname]',address='$_POST[address]',state='$_POST[state]',country='$_POST[country]',contact_no='$_POST[contactno]',login_id='$_POST[loginid]',s_password='$_POST[password]'";
		if($_FILES['browse']['name']!=="")
		{
			$sql=$sql.",imgpath='$filename'";
		}
		
		$sql=$sql.",status='$_POST[status]' WHERE shop_id='$_GET[editid]'";
		mysqli_query($con,$sql);
		echo"<script>alert('Record updated successfully...')</script>";
		echo "<script>window.location='viewshopowner.php';</script>";		
	}
	else
	{
			$query=mysqli_query($con,"insert into shopowner(compname,address,state,country,contact_no,login_id,s_password,created_at,imgpath,status)VALUES('$_POST[compname]','$_POST[address]','$_POST[state]','$_POST[countryi]','$_POST[contactno]','$_POST[loginid]','$_POST[password]','$dt','$filename','$_POST[status]')");
			if(!$query)
			{
				...
				{
					echo "<script>alert('Shop Owner/Seller Record  inserted successfully..');</script>";
					echo "<script>window.location='shopowner.php';</script>";
				}
			}
			$subject = "Shopowner login credentials";
...
if(isset($_GET['editid']))
{
	$sql="SELECT * FROM shopowner WHERE shop_id='$_GET[editid]'";
...
?>
        ...
        <div id="content" class="float_r">
            ...
            <form action="" method="post" name="frmshopowner" onsubmit="return validateadmin()" enctype="multipart/form-data">
      ...
      <input type="password" name="cpassword" id="cpassword"style="width:300px;" value="<?php echo $res['s_password'];?>"/>
              ...
              <p>Image :<input type="file" name="browse" id="browse" /></p>
  ...
  <script type="application/javascript">
	...
	{
		if(document.frmshopowner.compname.value == "")
		{
			alert("Company Name should not be empty..");
			document.frmshopowner.compname.focus();
			return false;
		}
		else if(document.frmshopowner.address.value == "")
		{
			alert("Address should not be empty..");
			document.frmshopowner.address.focus();
			return false;
		}
		else if(document.frmshopowner.country.value ="")
		{
			alert(" Select Country....");
			document.frmshopowner.country.focus();
			return false;
		}	
		else if(document.frmshopowner.state.value="")
		{
			alert("Select State ..");
			document.frmshopowner.state.focus();
			return false;
		}	
		else if(document.frmshopowner.contactno.value == "")
		{
			alert("contactno should not be empty..");
			document.frmshopowner.contactno.focus();
			return false;
		}	
		else if(document.frmshopowner.contactno.value.length<10)
		{
			alert("Enter 10 digit contact number..");
			document.frmshopowner.contactno.focus();
			return false;
		}	
		/*else if(document.frmshopowner.loginid.value.length < 8)
		{
			alert("Loginid should be more than 8 characters.");
			document.frmshopowner.loginid.focus();
			return false;
		}*/
		else if(document.frmshopowner.password.value.length < 8)
		{
			alert("Password should be more than 8 characters.");
			document.frmshopowner.password.focus();
			return false;
		}
		else if(document.frmshopowner.password.value != document.frmadmin.cpassword.value)
		{
		...
		else if(document.frmshopowner.browse.value == "")
		{
			alert("Select Image for the product...");
			document.frmshopowner.browse.focus();
			return false;
		}
		else if(document.frmshopowner.status.value="")
		{
			alert("select status..");
			document.frmshopowner.status.focus();
			return false;
		}								
		else
		{
