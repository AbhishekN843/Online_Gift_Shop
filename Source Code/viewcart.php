...
{	
	 $checkcart="SELECT * FROM purchase WHERE prod_id='$_GET[productid]' AND cust_id='$_SESSION[cid]' AND purchasestatus='Pending'";
	 $rescart=mysqli_query($con,$checkcart);
	 if(mysqli_num_rows($rescart) == 0)
	 {
	 		$cartquery=mysqli_query($con,"insert into purchase(prod_id,size_id,cust_id,bill_id,qty,price,comments,purchasestatus) VALUES('$_GET[productid]','$_GET[size]','$_SESSION[cid]','0','$_GET[qty]','$_GET[price]','','Pending')");
			if(!$cartquery)
			{
