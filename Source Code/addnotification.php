...
{
$squery="INSERT INTO  notification(custid,prod_id,notification_status)VALUES('$_POST[custid]','$_POST[prodid]','$_POST[notifystatus]')";
			...
			echo "<option value='$nqres[custid]' >$nqres[custfname]</value>";
              ...
              <td>Product ID</td>       
                <td><select name="prodid">
                 <option value="">Select</option>
                 <?php
			  $nsql2="SELECT * FROM products";
			  $nres2=mysqli_query($con,$nsql2);
			  while($nqres2=mysqli_fetch_array($nres2))
			  {
			echo "<option value='$nqres2[prod_id]' >$nqres2[prodname]</value>";
  ...
  ?>
   <script type="application/javascript">
            ...
            
	{
		...
		else if(document.notification.prodid.value == "")
		{
			alert(" select a product id..");
			document.notification.prodid.focus();
			return false;
		}
